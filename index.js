...
module.exports = extractDomContent
...
function extractDomContent(originalNode, options = {}) {
    ...
    for (let i = 0; i < allOriginalNodes.length; i++) {
        allClonedNodes[i].setAttribute('style', window.getComputedStyle(allOriginalNodes[i]).cssText);
    }

    // Get rid of unwanted elements for copy text
    for (const unwantedNode of mainNode.querySelectorAll('script, style, noscript, code')) {
        unwantedNode.remove();
    }
